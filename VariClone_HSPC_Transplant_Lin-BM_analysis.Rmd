...
Col_type <- c(rep("character", 3), rep("numeric", 52))

#Load data
all_data <- read.csv("Summary_of_all_Confetti_data/Sum_of_TransplantBM_Stem.csv", stringsAsFactors=FALSE, header=TRUE, colClasses= Col_type)
...
#Exp to analyze
A28 <- filter(all_data, Exp == Cohort & Genotype == "KO")
...
cell_type <- c("LK", "LSK", "LTHSC", "STHSC", "MPP2", "MPP3", "MPP4_CD48")
...
Cohort <- data.frame(A28$Group, A28$Mouse.Tag) # change here for Cohort or Ana_Cohort
